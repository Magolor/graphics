...
<scene>
""".replace("<camera>",camera).replace("<W>",str(args.W)).replace("<H>",str(args.H)).replace("<softlights>",softlights).replace("<texture>",args.texture).replace("<color>",args.color).replace("<scene>",scene)
)