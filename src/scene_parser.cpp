...
Vector3f SceneParser::rayTracing(const Ray &r, const Vector3f &background, int depth, double weight, double eps) const {
    Hit h = Hit(); if(weight < 1e-6) depth = 0; if( !group->intersect(r, h, 0) ){ if(texture->data==nullptr) return background;
...
void SceneParser::render(const char *filename, int depth, bool preview) const {
    Ray r; Vector3f c; tqdm bar; int K = 1, W = camera->w, H = camera->h, G = W*H*4, T = 0; Image *img; bool f=true;
    !(W&1)&&!(H&1)?W>>=1,H>>=1,K++:0; !(W&1)&&!(H&1)?W>>=1,H>>=1,K++:0; !(W&1)&&!(H&1)?W>>=1,H>>=1,K++:0;
