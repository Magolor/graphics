...
bool SceneParser::shadowTracing(const Ray &r, const Hit &h, const Vector3f &dirToLight, double eps) const {
    Vector3f p = r.p(h.t); p = p+(Vector3f::dot(dirToLight,h.normal)<0.?-eps:eps)*h.normal; Hit h2; return group->intersect(Ray(p,dirToLight), h2, 1e-9) && h2.t<dirToLight.length()-1e-9;
}
...
Vector3f SceneParser::rayTracing(const Ray &r, const Vector3f &background, int depth, double weight, double eps) const {
    ...
    Vector3f reflection = depth>0&&h.material->albedo[2]>1e-9?rayTracing(Ray(Ro,Rd.normalized()),background,depth-1,weight*h.material->albedo[2],eps):Vector3f::ZERO;
