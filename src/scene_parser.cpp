...
Vector3f SceneParser::rayTracing(const Ray &r, const Vector3f &background, int depth, double eps) const {
    Hit h = Hit(); if( !group->intersect(r, h, 0) ) return background; Vector3f p = r.p(h.t), d, lc; Light *l;
